...
describe 'foreman::config::passenger' do
  ...
  describe 'without parameters' do
    ...
    it 'should include apache with passenger' do
      should contain_class('apache')
      should contain_class('apache::mod::passenger')
    end

    it 'should ensure ownership' do
      should contain_file("#{params[:app_root]}/config.ru").with({
        :owner => params[:user],
      })

      should contain_file("#{params[:app_root]}/config/environment.rb").with({
        :owner => params[:user],
      })
    end
  end
